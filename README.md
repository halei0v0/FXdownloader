...
4. **管理** - 通过左侧导航访问历史记录、收藏夹、暂停任务
5. **批量** - 将多本小说加入购物篮，一键批量下载
...
│   ├── sudugu_rankings.py  # 速读谷排行榜+分类（按天缓存）
